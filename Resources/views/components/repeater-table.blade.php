<x-dynamic-component :component="$getFieldWrapperView()" :id="$getId()" :label="$getLabel()" :label-sr-only="$isLabelHidden()" :helper-text="$getHelperText()"
    ...
    <div
        ...
        <div @class([
            ...
            'sm:border sm:border-gray-300 dark:sm:border-gray-700' =>
                !$hasContainers && $breakPoint === 'sm',
            'md:border md:border-gray-300 dark:md:border-gray-700' =>
                !$hasContainers && $breakPoint === 'md',
            'lg:border lg:border-gray-300 dark:lg:border-gray-700' =>
                !$hasContainers && $breakPoint === 'lg',
            'xl:border xl:border-gray-300 dark:xl:border-gray-700' =>
                !$hasContainers && $breakPoint === 'xl',
            '2xl:border 2xl:border-gray-300 dark:2xl:border-gray-700' =>
                !$hasContainers && $breakPoint === '2xl',
        ])>
            <table class="w-full">
                <thead @class([
                    ...
                    <tr class="text-sm md:divide-x md:rtl:divide-x-reverse md:divide-gray-300 dark:md:divide-gray-700">
                        @foreach ($headers as $key => $header)
                            <th @class([
                                ...
                                @if ($columnWidths && isset($columnWidths[$key])) style="width: {{ $columnWidths[$key] }}" @endif>
                                {{ $header }}
                            </th>
                        @endforeach
                        @if ($hasActions)
                            <th
                                class="w-px p-2 filament-table-repeater-header-column bg-gray-200/50 dark:bg-gray-900/60 ltr:rounded-tr-xl rtl:rounded-tl-xl">
            ...
            <tbody wire:sortable
                ...
                @if (count($containers))
                    @foreach ($containers as $uuid => $row)
                        <tr wire:key="{{ $this->id }}.{{ $row->getStatePath() }}.item"
                            ...
                            @foreach ($row->getComponents() as $cell)
                                @if (!$cell instanceof \Filament\Forms\Components\Hidden && !$cell->isHidden())
                                    <td @class([
                                        ...
                                        @if ($columnWidths && isset($columnWidths[$cell->getName()])) style="width: {{ $columnWidths[$cell->getName()] }}" @endif>
                                        {{ $cell }}
                                    </td>
                                @else
                                    {{ $cell }}
                            ...
                            @if ($hasActions)
                                <td class="w-px p-2 filament-table-repeater-column">
                                    <div class="flex items-center md:justify-center">
                                        @unless ($isItemMovementDisabled)
                                            <button
